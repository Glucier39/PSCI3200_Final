...
```
Initial Fixed Effect Regression
```{r}
...
model_base <- lm(execorr_stag ~ year_count + gdp_log + expofgdp_log + trade_exp_log +  Country, data = comb)
summary(model_base)
```
